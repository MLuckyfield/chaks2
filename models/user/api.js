        ...
        users.forEach((user, i) => {
          ...
          if(user.statistics){
            if(user.statistics.length>0){
              ...
              console.log('User:',user.first,moment(user.statistics[0].start).format('MM D'),moment(user.statistics.reverse()[0].end).format('MM D'),moment(new Date()).diff(moment(user.statistics[0].end),'days'))
            }
          }
        });
      })
