...
const Statistics = (props)=>{
  ...
  const updateGoals =(e,id)=>{
    ...
    console.log(user)
    axios.post('user/goals',{filter:{_id: user},data:{'$push':{goals:id}}})
      .then((update)=>{
            ...
            <table style={{border:'none',width:'100%',height:'100%'}}>
                <tr style={{background:'none',verticalAlign:'middle',display:'table'}} >
                  <td style={{width:'80%'}}>
                    ...
                    <tr style={{background:'none',verticalAlign:'middle',display:'table'}}><td>{console.log('recieved',limit)}{limit<4?<button style={{background:'yellow',color:'white',width:'50%'}} onClick={(e)=>{updateGoals(e,content[0]._id)}}>Set Goal</button>:''}</td></tr>
                  </td>
